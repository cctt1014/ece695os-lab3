...
void ProcessModuleInit () {
  ...
  for (i = 0; i < PROCESS_MAX_PROCS; i++) {
    ...
    for (j = 0; j < MEM_L1TABLE_SIZE; j++) {
      pcbs[i].pagetable[j] = MemorySetupPte(pcbs[i].pagetable[j]);
    }
  ...
  for (i = 0; i < MEM_L1TABLE_SIZE; i++) {
    if ( (pcb->pagetable[i] & (uint32)(MEM_PTE_VALID)) != 0 ) {
      for(j = 0; j < MEM_L2TABLE_SIZE; j++) {
        if( *((uint32 *) (pcb->pagetable[i] & MEM_PTE_MASK) + j)  != 0) {
          MemoryFreePte (((uint32 *) (pcb->pagetable[i] * MEM_PTE_MASK)) + j );
        }
      }
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  stackframe        = ((pcb->sysStackArea) + (MEM_PAGESIZE-1)) & ~(0x3);
    
  // // User stack
  // // Get an available ppage and set the valid bit
  // pcb->pagetable[MEM_L1TABLE_SIZE-1] = MemorySetPte(MemoryAllocPage());

  // // Assign 4 pages
  // for (i = 0; i < 4; i++) {
  //   pcb->pagetable[i] = MemorySetPte(MemoryAllocPage());
  //   // printf("[DBG] allocated pte=%x\n", pcb->pagetable[i]);
  // }
  // printf("[DBG] sysStack_base_address=%d, userStack_pte=%d\n", pcb->sysStackArea, pcb->pagetable[MEM_L1TABLE_SIZE-1]);

  // Allocate first L2 page table
  pcb->pagetable[0] = MemorySetupPte(MemoryAllocPage());
  L2_pagetable = (uint32 *) (pcb->pagetable[0] & MEM_PTE_MASK);
  
  // Allocate 3 pages for text/data within first L2 page table
  for(i = 0; i < 3; i++) {
      *(L2_pagetable + i) = MemorySetupPte(MemoryAllocPage());
  ...
  L2_pagetable = (uint32 *) (pcb->pagetable[MEM_L1TABLE_SIZE - 1] & MEM_PTE_MASK);
  
  // Allocate 1 page for user stack within L2 last page table
  *(L2_pagetable + MEM_L2TABLE_SIZE - 1) = MemorySetupPte(MemoryAllocPage());
  ...
  stackframe = ((uint32 *)(pcb->sysStackArea + MEM_PAGESIZE)) - (PROCESS_STACK_FRAME_SIZE + 8);
  ...
  stackframe[PROCESS_STACK_PTSIZE] = MEM_L1TABLE_SIZE * MEM_L2TABLE_SIZE;
  ...
  if (isUser) {
    ...
    // printf("[DBG] stackframe[PROCESS_STACK_USER_STACKPOINTER] = %x\n", stackframe[PROCESS_STACK_USER_STACKPOINTER]);
    ...
    // printf("[DBG] IAR=%x, ISR=%x\n", stackframe[PROCESS_STACK_IAR], stackframe[PROCESS_STACK_ISR]);
  } else {
