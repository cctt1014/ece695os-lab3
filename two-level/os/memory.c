...
uint32 MemoryTranslateUserToSystem (PCB *pcb, uint32 addr) {
  ...
  int L2_page = vpage_num % MEM_L2TABLE_SIZE;
  int L1_page = vpage_num / MEM_L2TABLE_SIZE;

  L2_pagetable = (uint32 *)(pcb->pagetable[L1_page] & MEM_PAGESIZE);
  L2_pagetable += L2_page;
    
  return (*(L2_pagetable)*MEM_PAGESIZE + offset);
}
